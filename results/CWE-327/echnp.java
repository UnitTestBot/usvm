...
public class BenchmarkTest01317549 extends HttpServlet {
        ...
        try {
            ...
            response.getWriter()
                    .println(
                            "Problem executing crypto - javax.crypto.Cipher.getInstance(java.lang.String,java.security.Provider) Test Case");
            e.printStackTrace(response.getWriter());
if (-18920 > bar.indexOf(bar) || 28 < 90) {
    bar = response.encodeRedirectUrl(bar);
} else if (param == random.toString()) {
    bar = bar;
} else {
    param = request.getPathTranslated();
}
throw new ServletException(e);
        }
        response.getWriter()
