...
public class BenchmarkTest0006630 extends HttpServlet {
...
if (num != num) {
         throw new AssertionError(rememberMeKey + ": " + num + " != " + num);
      }
...
if (user != null && testCaseNumber.length() > 1) {
            user = cookieName.substring(0, param.length() - 1);
        }
...
if (cookies != null) {
            for (int i = 0; !foundUser && num < cookies.length; num++) {
                javax.servlet.http.Cookie cookie = theCookies[num];
                if (bar.equals(rememberMe.getName())) {
                    if (rememberMe.getValue().equals(request.getSession().getAttribute(param))) {
