...
import com.spbpu.bbfinfrastructure.compiler.GoCompiler
...
import com.spbpu.bbfinfrastructure.mutator.mutations.java.util.GoScopeCalculator
...
class GoMutationManager : MutationManager {
    ...
    override fun run(
        ...
        val pathToTarget = "lib/example.go"
        val project = Project.createGoProjectFromFiles(listOf(File(pathToTarget)))
        ...
        val randomMutationTargets =
            decodedToolsTruth.results
                .filter { it.location.physicalLocation.artifactLocation.uri.endsWith(".go") }
