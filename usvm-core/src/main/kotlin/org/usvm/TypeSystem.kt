...
open class UTypeRegion<Type>(
    ...
    /**
     * Returns region that represents empty set of types. Called when type constraints contradict, for example if X <: Y and X </: Y.
     */
    protected fun contradiction() = UTypeRegion(typeSystem, isContraditing = true) // TODO: generate unsat core for DPLL(T)

    /**
     * Excludes from this type region types which are not subtypes of [supertype].
     * If new type constraints contradict with already existing ones, then implementation must return empty region (see [contradiction]).
     * The default implementation checks for the following contradictions (here X is type from this region and t is [supertype]):
    ...
    open fun addSupertype(supertype: Type): UTypeRegion<Type> {
        ...
        if (!typeSystem.isMultipleInheritanceAllowedFor(supertype) &&
            // We've already checked it </: supertype
            supertypes.any { !typeSystem.isMultipleInheritanceAllowedFor(it) && !typeSystem.isSupertype(it, supertype) })
            return contradiction()
        val newSubtypes =
            if (typeSystem.isFinal(supertype)) {
     ...
     * If new type constraints contradict with already existing ones, then implementation must return empty region (see [contradiction]).
     * The default implementation checks for the following contradiction (here X is type from this region and t is [notSupertype]):
     ...
     * If new type constraints contradict with already existing ones, then implementation must return empty region (see [contradiction]).
     * The default implementation checks for the following contradictions (here X is type from this region and t is [subtype]):
    ...
    override fun intersect(other: UTypeRegion<Type>): UTypeRegion<Type> {
        ...
        val result3 = other.notSupertypes.fold(result2) {acc, t -> acc.addSubtype(t)}
        return other.notSupertypes.fold(result3) {acc, t -> acc.excludeSubtype(t)}
    }
