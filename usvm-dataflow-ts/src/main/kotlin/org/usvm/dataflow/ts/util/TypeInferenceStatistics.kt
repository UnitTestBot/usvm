...
private fun getEtsClassType(method: EtsMethod, graph: EtsApplicationGraph): EtsClassType? {
    val enclosingClass = method.enclosingClass

    return if (enclosingClass.name == DEFAULT_ARK_CLASS_NAME || enclosingClass.name.isBlank()) {
        null
    } else {
        val classSignature = EtsClassSignature(enclosingClass.name, enclosingClass.file, enclosingClass.namespace)

        val clazz = graph.cp
            .classes
            .firstOrNull { it.signature == classSignature }
            ?: error("No class found in the classpath with signature $classSignature")
