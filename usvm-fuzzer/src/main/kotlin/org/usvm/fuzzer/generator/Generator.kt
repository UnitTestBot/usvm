    ...
    fun generate(depth: Int) =
        run {
            if (depth > GeneratorSettings.maxDepthOfObjectGeneration) {
                ...
                UTestValueRepresentation(UTypedTestNullExpression(ctx.jcClasspath.objectTypeWrapper()))
...
//                println("GENERATION ${this::class.java.name} depth $depth")
                generationFun.invoke(ctx, depth + 1).let {
                    it ?: UTestValueRepresentation(UTypedTestNullExpression(ctx.jcClasspath.objectTypeWrapper()))
                }.also {
//                    println("GENERATION FINISHED ${this::class.java.name} depth $depth")
    ...
    fun generate() =
        try {
            ...
            val res =
                if (debugMode) {
                    softTimeout = 1.hours.inWholeMilliseconds
                    generate(0)
                } else {
                    executeWithTimeout(GeneratorSettings.hardTimeoutForObjectGeneration.inWholeMilliseconds) {
                        generate(0)
                    }
                }
            when (res) {
