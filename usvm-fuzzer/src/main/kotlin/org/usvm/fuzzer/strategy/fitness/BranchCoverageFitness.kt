...
class BranchCoverageFitness : TestFitness {
    override fun getFitness(seed: Seed, targetInsts: List<JcInst>): Double {
        val coverage = seed.coverage?.keys?.toList() ?: error("Can't calc fitness for not executed seed")
        var res = 0.0
    ...
    private fun getDistanceBetween(trace: List<JcInst>, targetInst: JcInst, targetMethod: JcMethod): Int {
        ...
        val visitedInstructions = HashSet<JcInst>()
        ...
        while (que.isNotEmpty()) {
            ...
            if (parentInst in visitedInstructions) {
                continue
            }
            visitedInstructions.add(parentInst)
