...
class GoExprVisitor(
    ...
    override fun visitGoSliceExpr(expr: GoSliceExpr): UExpr<out USort> {
        val collection = expr.array.accept(this).asExpr(ctx.addressSort).let { if (isPointerConcrete(it)) deref(it, ctx.addressSort) else it }
    ...
    override fun visitGoFieldAddrExpr(expr: GoFieldAddrExpr): UExpr<out USort> {
        ...
        return scope.calcOnState {
            ...
            mkPointer(fieldType, fieldLValue)
        }
    }
    ...
    override fun visitGoIndexAddrExpr(expr: GoIndexAddrExpr): UExpr<out USort> {
        ...
        return scope.calcOnState {
            ...
            mkPointer(elementType, elementLValue)
        }
    }
    ...
    private fun visitIndexExpr(instance: GoValue, idx: GoValue): Pair<UHeapRef, UExpr<USizeSort>> {
        val array = instance.accept(this).asExpr(ctx.addressSort).let { if (isPointerConcrete(it)) deref(it, ctx.addressSort) else it }
    ...
    private fun isPointerConcrete(pointer: UHeapRef): Boolean {
        return scope.calcOnState {
            ...
            field is KBitVec32Value && field.intValue == 50541
        }
    }
