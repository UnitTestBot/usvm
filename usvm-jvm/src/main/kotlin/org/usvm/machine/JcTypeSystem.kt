...
class JcTypeSystem(
    ...
    override fun findSubtypes(type: JcType): Sequence<JcType> {
        if (MainConfig.hardTimeLimit > 0 &&
            System.currentTimeMillis() - time > MainConfig.hardTimeLimit) {
            throw TimeLimitExceededException("TIME LIMIT")
        }
        return when (type) {
