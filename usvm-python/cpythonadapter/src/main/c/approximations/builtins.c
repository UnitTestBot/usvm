...
PyObject *
SymbolicMethod_float(SymbolicAdapter *adapter, jobject self, PyObject *args, PyObject *kwargs) {
    assert(self == NULL && args && PyTuple_Check(args));
    if (PyTuple_Size(args) != 1 || kwargs)
        return Py_None;
    PyObject *arg = PyTuple_GetItem(args, 0);
    PyObject *result = adapter->symbolic_float_cast(adapter->handler_param, arg);
    return result;
}