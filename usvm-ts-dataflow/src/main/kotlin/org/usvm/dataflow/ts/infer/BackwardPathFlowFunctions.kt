...
import org.usvm.dataflow.ts.infer.BackwardPathTypeDomainFact.Zero
import org.usvm.dataflow.ts.infer.BackwardPathTypeDomainFact.TypedVariable
import org.usvm.dataflow.ts.util.fixAnyToUnknown

class BackwardPathFlowFunctions(
    val doAddKnownTypes: Boolean,
) : FlowFunctions<BackwardPathTypeDomainFact, EtsMethod, EtsStmt> {
    override fun obtainPossibleStartFacts(method: EtsMethod): Collection<BackwardPathTypeDomainFact> = listOf(Zero)

    override fun obtainSequentFlowFunction(
        current: EtsStmt,
        next: EtsStmt,
    ): FlowFunction<BackwardPathTypeDomainFact> = FlowFunction { fact ->
    ...
    private fun sequentZero(current: EtsStmt): List<BackwardPathTypeDomainFact> {
        ...
        val result = mutableListOf<BackwardPathTypeDomainFact>(Zero)
    ...
    private fun callZero(
        callStatement: EtsStmt,
    ): List<BackwardPathTypeDomainFact> {
        val result = mutableListOf<BackwardPathTypeDomainFact>(Zero)
