...
- Stable-site prevalence rather than repeated loop-event totals. The largest repeated groups were application callback or dispatch limitations: `BSTreeKV.compare` (390 events, 3 sites), iterator `next` (141 events, 2 sites), and `Array.isArray` (120 events, 1 site). Project-defined `Stack.pop` accounted for 18 events at 3 sites; it is not the built-in `Array.pop` target. No built-in `Array.shift` site was observed.
