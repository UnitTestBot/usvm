...
internal class UPersistentHashMapBuilder<K, V>(private var map: UPersistentHashMap<K, V>) : PersistentMap.Builder<K, V>, AbstractMutableMap<K, V>() {
    ...
    override fun build(): UPersistentHashMap<K, V> {
        map = if (node === map.node) {
            map
        } else {
            ownership = MutabilityOwnership()
            UPersistentHashMap(node, size)
        }
        return map
    }
    ...
    override fun putAll(from: Map<out K, V>) {
        ...
        val map = from as? UPersistentHashMap ?: (from as? UPersistentHashMapBuilder)?.build()
